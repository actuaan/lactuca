...
methods**: {meth}`lactuca.LifeTable.ax`, {meth}`lactuca.LifeTable.axy`,
...
element of `cashflow_times`; see [below](#due-via-explicit-times).
:::
...
the helper {func}`lactuca.generate_payment_times` generates a ready-to-use
...
- {func}`lactuca.generate_payment_times` — helper for building payment schedules
